...
if (!(isset($_SESSION["username"])) || $_SESSION["username"] == "") {
    ...
    <head>
        <title><?php echo $settings["instance_name"]; ?> - Administration - Reports - Monthly</title>
    ...
    <div id="container">
        ...
        <h3><a href="index.php">Administration</a> - Reports - Monthly</h3>

        <br/><br/><strong>Monthly Report for the period starting: <?php echo date("F Y", $starttime); ?> and
            ending: <?php echo date("F Y", $endtime); ?></strong><br/><br/>
        ...
        <table id="reporttable">
            ...
            while ($fromstamp <= $tostamp) {
                ...
                foreach ($roomname_array as $key => $roomid) {
                    $rescount = mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT reservationid, UNIX_TIMESTAMP(start), UNIX_TIMESTAMP(end), roomid, username, timeofrequest FROM reservations WHERE (UNIX_TIMESTAMP(start) >= " . $fromstamp . " AND UNIX_TIMESTAMP(start) < " . strtotime("+1 month", $fromstamp) . ") AND roomid=" . $roomid_array[$key] . $orderbystr . ";"));
                    echo "<td>" . $rescount . "</td>";
