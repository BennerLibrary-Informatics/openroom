...
if (!(isset($_SESSION["username"])) || $_SESSION["username"] == "") {
    ...
    $from = isset($_REQUEST["fromc"]) ? $_REQUEST["fromc"] : "";
    $to = isset($_REQUEST["toc"]) ? $_REQUEST["toc"] : "";
    ...
    <head>
        <title><?php echo $settings["instance_name"]; ?> - Administration - Reports - Cancellations - Daily</title>
        ...
        <table id="reporttable">
            ...
            while ($fromstamp <= $tostamp) {
                ...
                foreach ($roomname_array as $key => $roomid) {
                    $rescount = mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT reservationid, UNIX_TIMESTAMP(start), UNIX_TIMESTAMP(end), roomid, username, timeofrequest FROM cancelled WHERE (UNIX_TIMESTAMP(start) >= " . $fromstamp . " AND UNIX_TIMESTAMP(start) < " . strtotime("+1 day", $fromstamp) . ") AND roomid=" . $roomid_array[$key] . $orderbystr . ";"));
                    echo "<td>" . $rescount . "</td>";
